...
- **Matter Insights:** OCR-first evidence intelligence system accessible as an "Insights" tab in matter detail view. Supports drag-drop file upload (PDF, images, Word docs, text, audio), automatic text extraction/OCR via Gemini Vision, SHA-256 hashing, and AI-powered analysis using Claude Sonnet. Generates 6 insight types: themes, timeline, entities, contradictions, action items, and risks—all with document citations. Action items can be converted to board tasks. DB tables: `matter_assets`, `asset_text`, `text_anchors`, `text_chunks`, `insight_runs`, `insight_outputs`. Backend at `server/routes/insights.ts`, services at `server/services/ingestion-pipeline.ts`, `server/services/insights-analysis.ts`, `server/services/insights-storage.ts`. Frontend at `client/src/components/matter-insights.tsx`.
