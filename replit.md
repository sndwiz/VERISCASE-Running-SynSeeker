...
- **Modular Storage Layer (Feb 2026):** `server/storage/` directory — 19 domain-specific storage modules extracted from monolithic `server/dbStorage.ts` (4,547 lines). Each module is a class with bound methods combined via `server/storage/index.ts` into a single flat `storage` object. Modules: boards, ai, evidence, timeline, threads, clients (clients+matters), contacts, research, automations, detective, files, people, billing, calendar, approvals, documents (with ~1,870 lines of legal template data), forms, meetings, security. Shared utilities in `server/storage/base.ts`. Original `server/dbStorage.ts` retained for reference but no longer imported.
...
- **SynSeeker Investigation Engine:** Automated entity investigation module with 8 data sources (web scraping via cheerio, WHOIS/domain intel, OpenCorporates corporate filings, CourtListener federal court records, NPI registry, online reviews, social profiles, news coverage). Features AI analysis via Anthropic Claude, heuristic scoring fallback, entity connection mapping, and real-time SSE progress streaming. 4 pre-built investigation templates (medical lien, opposing counsel, corporate, expert witness). DB tables: investigations, investigation_findings, discovered_entities, entity_connections. Frontend dashboard at /synseeker with Shadcn components. Optional API keys: COURTLISTENER_API_KEY, GOOGLE_CSE_KEY + GOOGLE_CSE_ID, NEWSAPI_KEY.
...
- **Reports Hub:** Clio-style reports system with 14 pre-built reports across 5 categories (Billing, Productivity, Cases, AI Operations, Compliance). Features card/table catalog view, preview modal, detail view with summary stats bar, sortable data table, column visibility sidebar, pagination, text filter, and CSV export. Backend at `/api/reports/catalog` and `/api/reports/:reportId/data`.
...
- **Kill Switch (Security Lockdown):** Emergency app-wide lockdown system accessible from the header. Admin-only activation generates a 6-character recovery key (crypto.randomBytes). Activating stops all processing, locks matter permissions, enables audit logging, and deploys honeypot documents. Deactivation requires the recovery key. In-memory state with 5-second polling, security event logging to DB. Components: server/services/kill-switch.ts, server/routes/security.ts (kill-switch endpoints), client/src/components/kill-switch.tsx.
