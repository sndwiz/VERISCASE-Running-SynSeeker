...
**UI/UX Decisions:**
The system adopts a Monday.com-style board architecture, offering highly customizable boards with over 15 column types and dynamic column management. It includes an "Approval Column" for legal verification workflows, a collapsible sidebar with hierarchical Client→Matter→Boards tree navigation, and theme support (dark/light mode with teal accents). The interface is fully responsive (mobile-first) and utilizes Shadcn/ui and Radix UI components, styled with Tailwind CSS, for a consistent and modern aesthetic. The header collapses toolbar buttons into an overflow dropdown menu on mobile via `useIsMobile` hook. All pages use responsive padding (p-3 md:p-6), form grids stack to single column on mobile (grid-cols-1 md:grid-cols-2), tables are wrapped in overflow-x-auto for horizontal scrolling, and master-detail layouts switch from side-by-side to stacked (flex-col md:flex-row). The Shadcn sidebar converts to a Sheet overlay on mobile automatically. The board view features a single horizontal scroller with frozen left columns, a global sticky header, scroll navigation tools, per-group workflow status filters, and three density modes. The sidebar includes board search, pin-to-top, and recent boards tracking.
...
- **Global Search:** Unified cross-entity search across 12 entity types (clients, matters, contacts, tasks, boards, time entries, evidence, invoices, calendar events, threads, detective nodes, files) with relevance-based scoring, type filters, and debounced real-time results.
- **Custom Fields & Matter Templates:** Per-practice-area custom field definitions (10 field types) with entity-specific values. 6 pre-built matter templates (Personal Injury, Family Law, Criminal Defense, Real Estate, Corporate, Employment) that auto-create boards with groups, tasks, and columns when applied to a matter.
- **Trust/IOLTA Three-Way Reconciliation:** Bank statement import (JSON/CSV), auto-matching entries to trust transactions by amount+date, manual matching, adjustments, and three-way balance verification (Bank Statement vs Book Balance vs Client Ledger). Includes full reconciliation reporting.
- **Data Export & Import Wizard:** One-click full firm JSON export of 17 entity types, single-matter export, and CSV export per entity type. Multi-format import wizard supporting VeriCase, Clio, MyCase, and PracticePanther data with field mapping, duplicate detection, validation preview, and rollback support.
- **Client Portal Management:** Portal access management with token-based client authentication, granular permissions, secure bidirectional messaging, document sharing with download tracking, and portal analytics dashboard.
- **E-Sign (Electronic Signatures):** Full envelope workflow with multi-signer support, signing order, dual-mode signature pad (draw or type), comprehensive audit trail with IP/user-agent tracking, and auto-completion when all signers sign.
- **SMS/Texting Integration:** Twilio-powered SMS with chat-style conversation UI, contact list, client/matter linking, and graceful fallback when Twilio is not configured (stores messages locally).
- **Email & Calendar Sync:** Integration infrastructure for Google and Microsoft 365 with connection management, sync settings, and synced email storage with matter linking.
- **Accounting Integration:** QuickBooks and Xero connection infrastructure with chart of accounts mapping and invoice/payment sync settings.
